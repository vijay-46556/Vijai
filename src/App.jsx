  ...
  const send = async () => {
    ...
    try {
      ...
      const res = await fetch(workerUrl, {
        ...
        body: JSON.stringify({
          system:"You are "+settings.aiName+", a next-generation AI assistant. Be intelligent, helpful and sharp. Use markdown when helpful. Never mention Claude or Anthropic.",
      ...
      const errMsg = { role:"assistant", content:"Connection error. Try again.", ts:Date.now() };
  ...
  if (screen==="login"||screen==="register") return (
    <div style={{minHeight:"100vh",background:C.bg,color:C.text,fontFamily:"'Segoe UI',system-ui,sans-serif",display:"flex",alignItems:"center",justifyContent:"center",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{background:C.surface,border:"1px solid "+C.border,borderRadius:20,padding:"36px 28px",width:"100%",maxWidth:400,position:"relative",zIndex:1,margin:"0 16px"}}>
        ...
        <p style={{textAlign:"center",color:C.textMuted,fontSize:12}}>Free: {FREE_LIMIT} msgs/day · Pro Rs.{settings.price}/month</p>
      </div>
    </div>
  );
      ...
      <div style={{flex:1,overflowY:"auto",padding:24}}>
        ...
        {adminTab==="settings"&&(
          <div style={{maxWidth:480}}>
            ...
            {[{label:"AI Name",key:"aiName"},{label:"Tagline",key:"tagline"},{label:"Welcome Message",key:"welcomeMsg"},{label:"UPI ID",key:"upiId"},{label:"Worker URL",key:"workerUrl"}].map(f=>(
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:18}}>
              {[{label:"Free Limit/Day",key:"freeLimit"},{label:"Pro Price Rs.",key:"price"}].map(f=>(
                <div key={f.key}>
                  <label style={{fontSize:11,color:C.textMuted,display:"block",marginBottom:5,textTransform:"uppercase",letterSpacing:0.8}}>{f.label}</label>
                  <input type="number" value={settings[f.key]} onChange={e=>setSettings({...settings,[f.key]:parseInt(e.target.value)||0})} style={{width:"100%",padding:"10px 13px",background:C.bg,border:"1px solid "+C.border,borderRadius:10,color:C.text,fontSize:13,outline:"none",fontFamily:"inherit"}}/>
                </div>
              ))}
            </div>
    ...
    <div style={{display:"flex",height:"100vh",background:C.bg,color:C.text,fontFamily:"'Segoe UI',system-ui,sans-serif",overflow:"hidden"}}>
      <div style={{width:245,flexShrink:0,background:C.surface,borderRight:"1px solid "+C.border,display:"flex",flexDirection:"column",padding:"13px 10px",gap:7,transition:"margin 0.3s",marginLeft:sidebar?0:-245,overflow:"hidden"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:9,padding:"9px",background:C.surface2,borderRadius:10}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            ...
            <div style={{fontSize:10,color:user?.pro?C.gold:C.textMuted,fontWeight:700}}>{user?.pro?"Pro":Math.max(0,remaining)+" msgs left"}</div>
          </div>
        </div>
        <div style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column",gap:2}}>
          {chats.map(c=>(
            <div key={c.id} onClick={()=>setActiveId(c.id)} style={{display:"flex",alignItems:"center",gap:7,padding:"8px 9px",borderRadius:8,cursor:"pointer",background:activeId===c.id?C.surface2:"transparent",color:activeId===c.id?C.text:C.textMuted,fontSize:12}}>
              <ChatIco/>
              <span style={{flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{c.title||"New Chat"}</span>
              <button onClick={e=>{e.stopPropagation();delChat(c.id);}} style={{background:"none",border:"none",color:C.textDim,cursor:"pointer",display:"flex",padding:2}}><Trash/></button>
            </div>
          ))}
        </div>
        {!user?.pro&&(
          <button onClick={()=>setUpgradeOpen(true)} style={{display:"flex",alignItems:"center",justifyContent:"center",gap:6,padding:"9px",background:"rgba(255,215,0,0.07)",border:"1px solid rgba(255,215,0,0.15)",borderRadius:10,color:C.gold,cursor:"pointer",fontSize:12,fontWeight:700}}>
            <Crown/>Upgrade Pro Rs.{settings.price}/mo
          </button>
        )}
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minWidth:0}}>
        <div style={{display:"flex",alignItems:"center",gap:10,padding:"12px 16px",borderBottom:"1px solid "+C.border,background:C.surface,flexShrink:0}}>
          ...
          <div style={{flex:1}}>
            <div style={{fontSize:16,fontWeight:800}}>Vij<span style={{color:C.accent}}>AI</span></div>
            <div style={{fontSize:10,color:C.textMuted}}>{settings.tagline}</div>
          </div>
          {user?.pro
            ?<div style={{display:"flex",alignItems:"center",gap:4,fontSize:11,color:C.gold,fontWeight:700,padding:"4px 10px",background:"rgba(255,215,0,0.07)",borderRadius:20,border:"1px solid rgba(255,215,0,0.15)"}}><Crown/>Pro</div>
            :<div style={{fontSize:11,color:C.textMuted,padding:"4px 10px",background:C.surface2,borderRadius:20,border:"1px solid "+C.border}}>{remaining} left</div>
          }
        </div>
        <div style={{flex:1,overflowY:"auto",padding:"20px 16px",display:"flex",flexDirection:"column",gap:16}}>
          {!activeChat?.messages?.length&&(
            <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",flex:1,textAlign:"center",gap:14,padding:"40px 16px"}}>
              <div style={{width:66,height:66,borderRadius:18,background:"linear-gradient(135deg,"+C.accent+","+C.accent2+")",display:"flex",alignItems:"center",justifyContent:"center",fontSize:28,fontWeight:900}}>V</div>
              <h2 style={{fontSize:22,fontWeight:900}}>I am {settings.aiName}</h2>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:8,justifyContent:"center",maxWidth:460}}>
                {["Explain quantum physics","Write Python code","Plan a trip to Goa","Best business ideas"].map(s=>(
                  <button key={s} onClick={()=>{setInput(s);taRef.current?.focus();}} style={{padding:"7px 13px",background:C.surface2,border:"1px solid "+C.border,borderRadius:18,color:C.textMuted,cursor:"pointer",fontSize:12,fontFamily:"inherit"}}>{s}</button>
                ))}
              </div>
            </div>
          )}
          {activeChat?.messages?.map((m,i)=>(
            <div key={i} style={{display:"flex",gap:9,alignItems:"flex-start",flexDirection:m.role==="user"?"row-reverse":"row",maxWidth:800,width:"100%",margin:"0 auto"}}>
              <div style={{width:30,height:30,borderRadius:9,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",background:m.role==="user"?C.surface2:"linear-gradient(135deg,"+C.accent+","+C.accent2+")",border:m.role==="user"?"1px solid "+C.border:"none",color:m.role==="user"?C.textMuted:"#fff",fontSize:11,fontWeight:900}}>
                {m.role==="user"?<UserIco/>:"V"}
              </div>
          ...
          {loading&&(
            <div style={{display:"flex",gap:9,alignItems:"flex-start",maxWidth:800,width:"100%",margin:"0 auto"}}>
        ...
        <div style={{padding:"13px 16px",borderTop:"1px solid "+C.border,background:C.surface,flexShrink:0}}>
          {!user?.pro&&msgCount>=FREE_LIMIT&&(
            <div style={{background:"rgba(255,71,87,0.07)",border:"1px solid rgba(255,71,87,0.18)",borderRadius:10,padding:"9px 14px",marginBottom:10,fontSize:13,color:"#ff6b7a",textAlign:"center"}}>
              Daily limit reached
              <button onClick={()=>setUpgradeOpen(true)} style={{background:"none",border:"none",color:C.accent,cursor:"pointer",fontWeight:700,fontSize:13,marginLeft:6}}>Upgrade Rs.{settings.price}/mo</button>
            </div>
          )}
          <div style={{display:"flex",gap:8,alignItems:"flex-end",background:C.surface2,border:"1px solid "+C.border,borderRadius:12,padding:"8px 10px"}}>
            <textarea
              ...
              disabled={!user?.pro&&msgCount>=FREE_LIMIT}
              placeholder={!user?.pro&&msgCount>=FREE_LIMIT?"Upgrade to continue...":"Message "+settings.aiName+"..."}
              style={{flex:1,background:"transparent",border:"none",color:C.text,fontSize:14,resize:"none",outline:"none",lineHeight:1.6,fontFamily:"inherit",maxHeight:140,minHeight:22}}
            />
            <button
              onClick={send}
              disabled={loading||!input.trim()}
              style={{width:34,height:34,borderRadius:9,background:"linear-gradient(135deg,"+C.accent+","+C.accent2+")",border:"none",color:"#fff",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",opacity:loading||!input.trim()?0.4:1,flexShrink:0}}>
              <Send/>
            </button>
          </div>
          <p style={{textAlign:"center",color:C.textDim,fontSize:10,marginTop:6}}>{settings.aiName} can make mistakes. Verify important info.</p>
        </div>
      </div>
      {upgradeOpen&&(
        <div onClick={()=>setUpgradeOpen(false)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.72)",backdropFilter:"blur(6px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:16}}>
          <div onClick={e=>e.stopPropagation()} style={{background:C.surface,border:"1px solid "+C.border,borderRadius:20,padding:"28px 24px",width:"100%",maxWidth:380,position:"relative"}}>
            <button onClick={()=>setUpgradeOpen(false)} style={{position:"absolute",top:14,right:14,background:"none",border:"none",color:C.textMuted,cursor:"pointer"}}><X/></button>
            <div style={{textAlign:"center",marginBottom:18}}>
              <div style={{width:50,height:50,borderRadius:14,background:"linear-gradient(135deg,"+C.accent+","+C.accent2+")",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,fontWeight:900,margin:"0 auto 10px"}}>V</div>
              <h2 style={{fontSize:19,fontWeight:900}}>Upgrade to Pro</h2>
              <p style={{color:C.textMuted,fontSize:13,marginTop:3}}>Unlimited messages every day</p>
            </div>
            <div style={{background:C.surface2,border:"1px solid rgba(0,229,255,0.12)",borderRadius:12,padding:"14px",marginBottom:18}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
                <span style={{color:C.gold,fontWeight:700,display:"flex",alignItems:"center",gap:5}}><Crown/>Pro Plan</span>
                <span style={{fontSize:20,fontWeight:900}}>Rs.{settings.price}<span style={{fontSize:12,color:C.textMuted,fontWeight:400}}>/mo</span></span>
              </div>
              {["Unlimited messages daily","Priority responses","All future features","No ads ever"].map(f=>(
                <div key={f} style={{fontSize:12,color:C.textMuted,padding:"3px 0"}}>✓ {f}</div>
              ))}
            </div>
            {!upgraded?(
              <>
                <p style={{textAlign:"center",fontSize:13,color:C.textMuted,marginBottom:10}}>Pay Rs.{settings.price} to UPI: <strong style={{color:C.accent}}>{settings.upiId}</strong></p>
                <input
                  value={upiTx}
                  onChange={e=>setUpiTx(e.target.value)}
                  placeholder="Enter UPI Transaction ID"
                  style={{width:"100%",padding:"10px 13px",background:C.bg,border:"1px solid "+C.border,borderRadius:10,color:C.text,fontSize:13,outline:"none",fontFamily:"inherit",marginBottom:12}}
                />
                <button onClick={activateUpgrade} style={{width:"100%",padding:"11px",background:"linear-gradient(135deg,"+C.accent+","+C.accent2+")",border:"none",borderRadius:12,color:"#fff",fontSize:14,fontWeight:700,cursor:"pointer"}}>Activate Pro</button>
              </>
            ):(
              <div style={{textAlign:"center",color:C.green,fontSize:16,fontWeight:700,padding:"14px 0"}}>Pro Activated! Welcome!</div>
            )}
          </div>
        </div>
      )}
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .cb{background:#0d1117;border:1px solid #1e2d3d;border-radius:8px;padding:11px 13px;overflow-x:auto;font-family:monospace;font-size:12px;color:#e6edf3;margin:8px 0;}
        .ic{background:#1c2333;border-radius:4px;padding:2px 5px;font-family:monospace;font-size:12px;color:#79c0ff;}
